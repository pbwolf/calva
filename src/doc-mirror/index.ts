  ...
  edit(modelEdits: ModelEdit<ModelEditFunction>[], options: ModelEditOptions): Thenable<boolean> {
    ...
    if (!postEditPlan) {
      ...
      return editCompletion.then((isFulfilled) => {
        if (!isFulfilled) {
          console.warn('Commanded edit was not fulfilled');
        } else {
                ...
                return this.postEditReformat(editor, postEditPlan.reformatOffsets).then(
                  (reformatFulfilled) => {
                    if (!reformatFulfilled) {
                      console.warn('Post-edit auto-reformat was not fulfilled');
                    }
                    return true; // because the commanded edit is the important thing, and it was fulfilled
                  }
                );
              }
            }
